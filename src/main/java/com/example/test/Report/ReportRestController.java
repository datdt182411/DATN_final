...
//    private DateFormat dateFormatter;
    ...
    public List<ReportItem> getReportDataByDatePeriod(@PathVariable("period") String period) {
        ...
        switch (period) {
            case "today":
                return  masterOrderReportService.getReportDataToday(ReportType.DAY);
            case "last_7_days":
    ...
    public List<ReportItem> getReportDataByTypeOrProduct(@PathVariable("groupBy") String groupBy,
                            @PathVariable("period") String period) {
        ReportType reportType = ReportType.valueOf(groupBy.toUpperCase());

        switch (period) {
            case "today":
                return  orderDetailReportService.getReportDataToday(reportType);
            case "last_7_days":
